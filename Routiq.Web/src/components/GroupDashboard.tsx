...
import { runDecisionEngine } from '../api/routiqApi';
...
const DECISION_STORAGE_KEY = 'routiq_decisions';
                            ...
                            {!decisionResult ? (
                                <div className="text-center">
                                    ...
                                    <button onClick={handleRunEngine} disabled={isCalculating}
                                        className={`mt-3 px-6 py-2 font-bold rounded-lg transition-all shadow text-xs flex items-center justify-center gap-2 mx-auto ${isCalculating ? 'bg-indigo-800 text-white cursor-wait opacity-90' : 'bg-indigo-600 hover:bg-indigo-700 text-white'}`}>
                                        {isCalculating ? (
