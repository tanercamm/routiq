...
export const AnalyticsPage = () => {
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            if (!user) return;
            try {
                const response = await routskyApi.get('/analytics');
    ...
    if (!data) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4">
                ...
                <p className="text-gray-500 dark:text-gray-400 max-w-sm">Travel with Routsky to generate real insights about your journeys.</p>
            </div>
        );
    }
